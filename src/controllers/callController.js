    ...
    async getCallTranscription(req, res) {
        try {
            ...
            if (result.data.status === 'completed') {
                return res.status(result.status).json({ status: result.data.status, transcription: result.data.transcription });
            }

            return res.status(result.status).json({ status: result.data.status });
        } catch (err) {
            res.status(500).json({ error: err.message });
        }
    }
